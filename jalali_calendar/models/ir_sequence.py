...
class JalaliIrSequence(models.Model):
    ...
    def _get_prefix_suffix(self, date=None, date_range=None):
        ...
        def _interpolation_dict():
            ...
            for key, format in sequences.items():
                if key[0] == 'j':
                    res[key] = jdatetime.datetime.fromgregorian(date=effective_date).replace(
                        tzinfo=pytz.UTC).astimezone(pytz.timezone('Asia/Tehran')).strftime(format)
                    res['range_' + key] = jdatetime.datetime.fromgregorian(date=range_date).replace(
                        tzinfo=pytz.UTC).astimezone(pytz.timezone('Asia/Tehran')).strftime(format)
                    res['current_' + key] = jdatetime.datetime.fromgregorian(date=now).replace(
                        tzinfo=pytz.UTC).astimezone(pytz.timezone('Asia/Tehran')).strftime(format)
                else:
                    res[key] = effective_date.strftime(format)
                    res['range_' + key] = range_date.strftime(format)
